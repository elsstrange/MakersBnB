feature 'viewing booking requests for spaces' do
  ...
  let!(:cancelled_booking) { create(:rejected_booking, advance_date: 3, space_id: space2.id, guest_id: guest.id) }
  ...
  scenario 'a host reviews the booking requests they have received for their spaces' do
    ...
    within("section.bookings-as-host#space#{space1.id}") do
    ...
    within("section.bookings-as-host#space#{space2.id}") do
